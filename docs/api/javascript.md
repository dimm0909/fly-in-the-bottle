...
| `wingCommand` | `{ amp, fold }` или `null`: если задан, перебивает амплитуду и складывание крыльев; может нести раздельные команды `{ left: { amp, fold }, right: { amp, fold } }` (использует `BrainFly`). |
| `wingSide` | `[{ amp, fold }, { amp, fold }]`: состояние левого и правого крыла; `wingAmp` и `wingFold` - их средние. |
| `haltereCommand`, `haltereAmp` | `[левый, правый]`: цель и текущий размах гальтеров, 0-1; если команды нет, гальтер бьёт с 0,8 размаха своего крыла. |
| `leg.ext`, `leg.extV` | Положение стопы вдоль тела относительно стойки (в шагах, вперёд положительно) и его скорость; считает `poseLegs`, читает `BrainFly.senseLegs`. |
...
| `power`, `steer`, `headYaw`, `headPitch`, `abdCurl`, `proboscis` | Сглаженные команды от мозга. `power` - среднее `powerSide`; `steer` - асимметрия крыльев, положительный - налево. |
| `powerSide`, `wingGoal`, `haltereGoal` | Мощность левого и правого крыла; что сеть просит у каждого крыла (`{ amp, fold }`) и гальтера (0-1). Поворот в полёте считается из разности `wingGoal[0].amp` и `wingGoal[1].amp`. |
...
| `senseLegs(put)` | Токи стоящих ног: нагрузка (`cs_leg_*`), положение и скорость (`co_leg_*`), концы диапазона (`hp_leg_*`), остальное (`lg_leg_*`), касание опоры (`touch_leg_*`). |
