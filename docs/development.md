...
| Покой | 30 с без событий (`w.fly.nextAmbient = 1e9` отключает редкие случайные воздействия): муха стоит неподвижно |
| Случайные воздействия | 4 минуты как есть: раз в 5-15 с что-то случается; нет NaN, муха не выходит за стенки, взлёты не идут серией |
| Тычок и встряска | `w.fly.launch(new w.THREE.Vector3(1, 3, 0), 12)` или `w.motion.burst = 4`: тело вращается (`w.fly.omega`), мозг отвечает (`w.fly.power`), потом посадка |
| Ходьба | `w.fly.seeObject(0.8, 'R')` каждые 50 мс в течение секунды; смотреть `w.fly.walk`, `w.fly.turn` и `w.link.rate('dn_DNp09_R')` |
| Груминг | `w.fly.touch('c_head', 0.05, 1)`, потом 20-25 секунд: `w.fly.dust` падает от ~1 к нулю тремя-четырьмя ступенями, `w.fly.cleanBlend` вспыхивает примерно на секунду на каждой |
| Отрыв и посадка | `w.fly.threat.strength = 1; w.fly.threat.side = 'R'` на свежепосаженной мухе: `w.fly.state` становится `fly` через 0,06-0,09 с, `w.fly.jump` около 1, посадка через 1,5-2 с; касания (`w.fly.touch('c_thorax', 0)`) с пола муху не поднимают |
| Курсор мимо мухи | `PointerEvent('pointermove')` на `canvas` каждые 16 мс вдоль прямой на ~90 пикселей выше мухи (её положение: `w.fly.object.getWorldPosition(v).project(w.camera)`); смотреть `w.fly.seen`, `w.fly.threat`, `w.fly.state` |
...
| Кадр не рисуется, в консоли `fly.object.getWorldPosition is not a function` | Подкласс перезаписал поле `object` базового `Fly` (корневой узел сцены). Не называйте свои поля `object`, `model`, `pos`, `vel`, `quat`, `omega`: они заняты `Fly` (у `BrainFly` объект в поле зрения - `seen`). Чтобы поймать исключение, вызовите `w.frame(performance.now() + 1000)` в `try/catch` в тестовом скрипте. |
...
| Муха взлетает в начале каждого прохода курсора в тесте | Тест телепортировал муху (`beginStand`), пока указатель ещё «внутри»: положение мухи на экране скачет, и сближение считается нападением. Сначала пошлите `pointerleave` и подождите 300-400 мс, потом переставляйте муху, потом возвращайте указатель. |
